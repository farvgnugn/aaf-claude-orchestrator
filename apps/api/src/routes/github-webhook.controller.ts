...
function verifySig(secret: string, payload: string, signature?: string) {
  ...
  const hmac = createHmac('sha256', secret); const digest = 'sha256=' + hmac.update(payload).digest('hex')
  try { return timingSafeEqual(Buffer.from(digest), Buffer.from(signature)) } catch { return false }
}
...
export class GithubWebhookController {
  ...
  async handle(@Body() body: any, @Headers('x-hub-signature-256') sig?: string) {
    ...
    if (body.pull_request) {
      ...
      if (existing) await this.prisma.story_pull_requests.update({ where: { id: existing.id }, data })
      else await this.prisma.story_pull_requests.create({ data })
      return { ok: true }
    }
