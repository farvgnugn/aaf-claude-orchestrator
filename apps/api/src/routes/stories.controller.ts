...
export class StoriesController {
  ...
  async create(@Body() body: { epicId: number; title: string }) {
    ...
    return this.prisma.user_stories.create({ data: { public_id: randomUUID(), epic_id: epic.id, title: body.title, status: 'DRAFT' } })
  }

  @Get()
  list(@Query('epicId') epicId?: string) {
    if (epicId) return this.prisma.user_stories.findMany({ where: { epic_id: Number(epicId) } })
