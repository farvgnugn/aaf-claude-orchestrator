...
new QueueEvents('dev', connection); new QueueEvents('review', connection)

async function enqueueDevJobs() {
  const story = await prisma.user_stories.findFirst({ where: { status:'APPROVED', dependencies_met:true, epic: { project: { repo_ready:true } } }, include: { epic: { include: { project: true } } }, orderBy: { id: 'asc' } })
  if (!story) return
...
new Worker('dev', async job => {
  ...
  if (!artifact) artifact = await prisma.artifacts.create({ data: { public_id: crypto.randomUUID(), project_id: project.id, scope_kind:'STORY', scope_id: story.id, kind:'DEV_NOTES', title:'Dev Notes', current_version_no:0 } })
  ...
  try { await createBranch(owner, repo, branch, baseSha) } catch {}
  await putFile(owner, repo, branch, `.changes/dev-notes-${story.public_id}.md`, devNotes, `docs: add dev notes for ${story.public_id}`)
  ...
  await prisma.story_pull_requests.create({ data: { story_id: story.id, provider: 'github', repo: `${owner}/${repo}`, pr_number: pr.number, branch, status: 'OPEN', url: pr.html_url, head_sha: pr.head?.sha || null, base_branch: base } })
